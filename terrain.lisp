    ...
    (case water-at
      ...
      (:START
       (cond ((plusp range)
	     ...
	     ((minusp range)
	      (contour-offset (- (* depth (abs range) 2) depth)
			      (* depth (abs range) 2)
			      half-down-y)))))))
...
(defun water-offset-right (depth water-at b-r-ele r-t-ele hex-r)
  ...
  (let ((half-down-y (* +sin60+ hex-r))
    ...
    (case water-at
      ...
      (:END
       (cond ((plusp range)
	      (contour-offset (- (* depth range 2) depth)
			      (* depth range 2)
			      half-down-y))
	     ((zerop range)
	      (contour-offset 0 depth half-down-y))
